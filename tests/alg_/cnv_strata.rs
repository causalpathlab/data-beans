...
use data_beans::alg::collapse_data::{collapse_columns_multilevel_with_strata, MultilevelParams};
...
fn without_strata_private_program_leaks_into_delta_and_residual() {
    ...
    let proj = v
        ...
        .expect("proj")
        .proj;
    let out = data_beans::alg::collapse_data::collapse_columns_multilevel_with_hierarchy(
        &mut v,
        &proj,
        &batches,
        &params(),
...
fn a_single_stratum_matches_unstratified_hierarchy() {
    ...
    let proj = v
        ...
        .expect("proj")
        .proj;
    let out_none = data_beans::alg::collapse_data::collapse_columns_multilevel_with_hierarchy(
...
fn assert_collapsed_means_agree(
    a: &data_beans::alg::collapse_data::CollapsedOut,
    b: &data_beans::alg::collapse_data::CollapsedOut,
    ...
    let check = |name: &str,
                 xa: Option<&legume_numeric::param::dmatrix_gamma::GammaMatrix>,
                 xb: Option<&legume_numeric::param::dmatrix_gamma::GammaMatrix>| {
        match (xa, xb) {
            (None, None) => {}
            (Some(ga), Some(gb)) => {
