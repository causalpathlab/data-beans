...
# installs both, so there is no per-binary loop here. The backend / HDF5
# detection below is kept in step with the legume-rs Makefile.
...
# Try the requested backend first and fall back to a CPU install if the GPU
# build fails, so a default `make install` on a Linux box without CUDA
# libraries still succeeds.
install:
ifeq ($(BACKEND),cuda)
	@echo "CUDA compute capability: $(or $(CUDA_COMPUTE_CAP),none) ($(CUDA_CAP_SOURCE))"
endif
