...
fn streaming_fragments_progress() -> indicatif::ProgressBar {
    legume_numeric::matrix::progress::new_spinner("{spinner} streamed {pos} fragments ({per_sec})")
}
...
fn run_fragments_preload_parallel(
    ...
    let dec_pb = legume_numeric::matrix::progress::new_spinner(
        "{spinner} decompressing... {bytes} ({bytes_per_sec})",
    );
