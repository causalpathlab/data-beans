...
fn frontier_reaches_the_targets_with_nested_levels() {
    ...
    for l in 0..2usize {
        ...
        let (c, _) = crate::alg::dc_poisson::compact_labels(&side);
        assert!(
...
fn packed_codes_reproduce_the_levels() {
    ...
    for (lvl, &d) in [&fine, &mid, &coarse].iter().zip(&dims) {
        ...
        let (a, ka) = crate::alg::dc_poisson::compact_labels(&masked);
        let (b, kb) = crate::alg::dc_poisson::compact_labels(lvl);
        assert_eq!(ka, kb);
        assert!(
...
fn split_recovers_planted_program() {
    ...
    let (side, _) = crate::alg::dc_poisson::compact_labels(&out.labels_per_level[0]);
    assert!(
