...
pub(crate) fn ve_ratio(r: &DMatrix<f32>, labels: &[usize], top_q: usize) -> f32 {
    ...
    let (compact, k) = crate::alg::dc_poisson::compact_labels(labels);
    if k < 2 {
        return 1.0;
    }
...
pub(crate) fn bisect_to_target(
    ...
    let prog_bar = legume_numeric::matrix::progress::new_progress_bar(final_target)
        .with_message("pb tree leaves");
...
pub(crate) fn pack_levels(levels: &[Vec<usize>]) -> (Vec<usize>, Vec<usize>) {
    ...
    for (li, level) in levels.iter().enumerate() {
        ...
        let width: usize;
        if li == 0 {
            let (compact, k) = crate::alg::dc_poisson::compact_labels(&active);
            width = bits_for(k);
            field = compact;
        } else {
