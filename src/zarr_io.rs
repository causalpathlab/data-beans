...
/// any internal prefix produced by [`legume_numeric::matrix::common_io::zip_dir`] so
...
pub fn materialize_writable_backend(src: &str, dst: &str) -> anyhow::Result<()> {
    if src.ends_with(".zarr.zip") {
        ...
        legume_numeric::matrix::common_io::recursive_copy(src, dst)
    }
}
...
pub fn finalize_zarr_output(zarr_dir: &str, target_path: &str) -> anyhow::Result<()> {
    ...
    if let Err(e) =
        legume_numeric::matrix::common_io::zip_dir_as(zarr_dir, &tmp_target, Some(&stem))
    {
